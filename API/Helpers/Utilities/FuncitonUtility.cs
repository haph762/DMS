...
namespace API.Helpers.Utilities
{
    public interface IFunctionUtility
    {
        ...
        string GenerateCodeIdentity(string function, string code);
    }
    ...
    public class FunctionUtility : IFunctionUtility
    {
        ...
        private string GetSHA512(string text)
        {
            ...
            var hashString = System.Security.Cryptography.SHA512.Create();
            string hex = "";
        ...
        public string GenerateCodeIdentity(string function, string code)
        {
            return $"{function}{(Convert.ToInt32(code) + 1).ToString().PadLeft(5, '0')}";
        }

        public string HashId(int id)
        {
            return GetSHA512(id.ToString()).Substring(0, 25).ToString();
        }
    }
}